...
static void memory_copy(long unsigned int *dest, const long unsigned int *src, long unsigned int *dest_end)
{
	if (dest == src)
		return;
	while (dest < dest_end) {
...
__attribute__ ((section(".teensy_boot.vectors")))
const uint32_t vector_table[2] = {
		0x20010000,
	(uint32_t)&ResetISR_pre
};

__attribute__ ((section(".boot_hdr.boot_data1")))
...
__attribute__ ((section(".boot_hdr.ivt1") ))
...
__attribute__ ((section(".boot_hdr.conf1")))
